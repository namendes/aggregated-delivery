...
<#macro renderMenuItem item level=0>
  ...
  <#elseif item.properties??>
    <#local linkHref>${item.properties['linkUrl']!}</#local>
  </#if>

  <#local hasAnyChild = hasAnyMenuItemChild />
        ...
        <ul class="dropdown-menu">
          <#if hasAnyMenuItemChild>
            <#list item.childMenuItems as childMenuItem>
              <@renderMenuItem childMenuItem level+1 />
            </#list>
          </#if>
          <#if hasAnyExtResChild>
            <#list externalRefs as childExtResItem>
              <@renderMenuItem childExtResItem level+1 />
            </#list>
          </#if>
        </ul>
      </#if>
...
<#if menu??>
  ...
  <div role="navigation" class="bloomreach col-md-8 col-sm-8 col-xs-12" style="padding-left: 0px; padding-right: 0px;">
