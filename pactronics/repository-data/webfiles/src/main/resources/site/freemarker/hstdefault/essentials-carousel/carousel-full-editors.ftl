...
<#-- @ftlvariable name="item" type="com.bloomreach.commercedxp.demo.pactronics.beans.Banner" -->
...
<#if pageable?? && pageable.items?has_content>
            ...
            <#list pageable.items as item>
                ...
                <#-- button to edit shown item -->
                  ...
                  <#else>
                    ...
                    <img style="width: 100%; display: block;" src="${getImageUrl(item)}" alt="${item.title?html}">
                    <div class="row" style="position: absolute;top:40px;left:110px;width: 100%">
                        <div class="row">
                            <div class="col-md-6 col-sm-8 col-xs-8" id="shopnow">
                                <h1 class="banner-title">${item.title?html} </h1>
                            <@hst.html var="content" hippohtml=item.content/>
                                <div id="sub-content-text">${content}</div>
                            </div>
                        </div>
                        <div class="row">
    ...
    <#-- buttons for still available items -->
        <#if editMode>
            ...
            <#list 1..(configuredItems?size + freeItems?size) as index>
                <div class="has-new-content-button item-button">
                    <#if configuredItems?seq_contains(index)>
                        <@hst.manageContent documentTemplateQuery="new-banner-document" hippobean=pageable.items[itemCounter] parameterName="document${index}" rootPath="banners"/>
                        <#assign itemCounter = itemCounter + 1>
                    <#else>
                        <@hst.manageContent documentTemplateQuery="new-banner-document" parameterName="document${index}" rootPath="banners"/>
                    </#if>
                </div>
            </#list>
        </#if>
...
<#function getImageUrl(item)>
    <#assign bynderImage = item.getBynderImage() />
    <#if bynderImage?? && bynderImage?has_content>
        <#return bynderImage />
    <#else>
        <@hst.link hippobean=item.image var="galleryImage"/>
        <#return galleryImage />
    </#if>
</#function>

<#function getLink(item)>
    <#assign externalLink = item.getExternalLink() />
    <#if externalLink?? && externalLink?has_content>
        <#return externalLink />
    <#else>
        <@hst.link hippobean=item.link var="link" />
        <#return link />
    </#if>
</#function>
