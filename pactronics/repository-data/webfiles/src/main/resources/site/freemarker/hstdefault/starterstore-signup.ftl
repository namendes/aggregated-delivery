...
<div class="col-xs-12 col-md-offset-3 col-md-6">
  ...
  <form method="POST" action="<@hst.actionURL />">
    ...
    <div class="form-group <#if formErrors?? && formErrors.hasItem('password2')>has-error</#if>">
      <label class="control-label" for="password_confirmation">
        <@fmt.message key='signup.label.password2' />
      </label>
      <input type="password" class="form-control"
        name="password2" id="password_confirmation"
        placeholder="<@fmt.message key='signup.label.password2' />" />
      <#if formErrors?? && formErrors.hasItem('password2')>
        <#assign formErrorItem=formErrors.getItem('password2') />
        <small class="help-block">
          ${formErrorItem.message}
        </small>
      </#if>
